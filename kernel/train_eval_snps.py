...
def cross_validation_without_val_set( dataset,
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(
        ...
        model = Gene_ontology_network(A_g, A, 2, 2, [5, 5], pool_dim, l_dim, device)
        model.to(device)
        ...
        pbar = tqdm(range(1, epochs + 1), ncols=70)
        for epoch in pbar:
            train_loss, ll_class, ll_reg = train(model, optimizer, train_loader, device, criterion_class, criterion_recon, temperature, lambda0)
            test_loss, test_ll_class, test_ll_reg = eval_loss(model, test_loader, device, criterion_class, criterion_recon, temperature, lambda0)
            ...
            print_log = 'Fold: %d, epoch:%d, train_loss: %0.4f, test_loss: %0.4f, test_loss_1: %0.4f, test_loss_2: %0.4f, test_acc: %0.4f, test_auc: %0.4f, test_f1: %0.4f, test_sen: %0.4f, test_spe: %0.4f' % (
                fold, eval_info["epoch"], eval_info["train_loss"], eval_info["test_loss"],  test_ll_class, test_ll_reg, eval_info["test_acc"], eval_info["test_auc"], eval_info["test_f1"]
...
def train(model, optimizer, loader, device, criterion_class, criterion_recon, temperature, lambda0):
    ...
    for data, label in loader:
        ...
        latent, x_hat, prob = model(data, temperature, device)
        y_hat = model.classification(torch.cat((latent, data), -1))
        class_loss = torch.sum(criterion_class(y_hat.view(-1), label.view(-1)))  # classification loss

        recon_loss = lambda0 * torch.sum(criterion_recon(x_hat, data))  # Reconstruction loss which acts a regularization.
    ...
    ll_class = np.sum(losses_class)/ num_sample
    ll_reg = np.sum(losses_reg)/ num_sample

    return ll, ll_class, ll_reg
...
def eval_loss(model, loader, device, criterion_class, criterion_recon, temperature, lambda0):
    ...
    for data, label in loader:
        ...
        recon_loss = lambda0 * torch.sum(
            criterion_recon(x_hat, data))  # Reconstruction loss which acts a regularization.
